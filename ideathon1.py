...
if climate_data_file and province_detail_file and station_detail_file:
    ...
    if set(features).issubset(final_dataset.columns) and target in final_dataset.columns:
        ...
        shap_fig = shap.force_plot(
            ...
            shap_values[feedback_index].values,
            X_test.iloc[feedback_index],
            matplotlib=True
        )
        st.pyplot(shap_fig)

    else:
        st.write("The dataset does not contain the required features or target variable.")

else:
    st.write("Please upload all the required CSV files to proceed.")
